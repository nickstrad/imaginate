...
- **`/plan-archive`** — the skill at `.claude/skills/plan-archive/SKILL.md` loads `docs/AGENTS.md`, `docs/plans/AGENTS.md`, `architecture.md`, `code-style/AGENTS.md`, and `testing/AGENTS.md`, then audits `docs/plans/open/` against the retirement policy and proposes archive/delete/keep decisions.
- **`/plans-refresh`** — the skill at `.claude/skills/plans-refresh/SKILL.md` loads the same docs as `/plan-archive`, then audits each open plan against the **current codebase** and the **other open plans** to find stale, partially-implemented, or superseded plans. Proposes per-plan decisions: keep, shrink, salvage-and-merge, supersede, or delete.

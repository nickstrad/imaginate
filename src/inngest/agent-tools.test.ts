...
describe("applyEdit", () => {
  it("rejects empty find", () => {
    const r = applyEdit("abc", { find: "", replace: "x", expectedOccurrences: 1 });
    expect(r.ok).toBe(false);
  });
  it("rejects when find is not present", () => {
    const r = applyEdit("abc", { find: "xyz", replace: "q", expectedOccurrences: 1 });
    expect(r.ok).toBe(false);
  });
  ...
  it("replaces multiple when expectedOccurrences matches", () => {
    const r = applyEdit("a-a-a", { find: "a", replace: "b", expectedOccurrences: 3 });
    expect(r).toEqual({ ok: true, content: "b-b-b", count: 3 });
  });
  it("fails when expectedOccurrences mismatches actual count", () => {
    const r = applyEdit("a-a-a", { find: "a", replace: "b", expectedOccurrences: 1 });
    expect(r.ok).toBe(false);
    if (!r.ok) {

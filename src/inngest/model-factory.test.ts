...
import { resolveSpecWith, MODEL_REGISTRY, EXECUTOR_LADDER } from "./model-factory";

describe("resolveSpecWith", () => {
  it("returns the requested provider when its key exists", () => {
    const cfg = resolveSpecWith(
      { provider: "openai", model: "gpt-5" },
      (p) => (p === "openai" ? "sk-openai" : undefined),
    );
    expect(cfg).toEqual({ provider: "openai", model: "gpt-5", apiKey: "sk-openai" });
  });
